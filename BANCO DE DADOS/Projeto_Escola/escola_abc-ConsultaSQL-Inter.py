...
try:  # O bloco try tenta criar uma conexão com o banco MySQL usando as credenciais informadas
    ...
    while True:
        ...
        print("2) Listar professores por inicial do nome")
        print("3) Executar SELECT livre (somente leitura)")
        print("0) Sair")

        opcao = input('Escolha uma opção: ').strip()

        if opcao == '1':
            consultar_tabela(conexao, 'SELECT * FROM ALUNO',
                             'ALUNOS CADASTRADOS:')

        elif opcao == "2":
            inicial = input("Digite a inicial (ex.: A): ").strip()
            if not inicial:
                print("Inicial vazia. Operação cancelada.")
            else:
                sql = f"SELECT * FROM professor WHERE nome LIKE '{inicial}%' ORDER BY nome"
                consultar_tabela(
                    conexao, sql, f"Professores com inicial '{inicial.upper()}'")

        elif opcao == "3":
            print("\nDigite uma consulta SOMENTE-LEITURA (apenas SELECT).")
            print("Ex.: SELECT * FROM aluno LIMIT 5;")

            sql_livre = input('SQL> ').strip().rstrip(";").lower()

            if not sql_livre:
                print("Consulta vazia. Operação cancelada.")

            elif not sql_livre.startswith('select'):
                print("Apenas consultas SELECT são permitidas nesta opção.")
            else:
                consultar_tabela(conexao, sql_livre, "Resultado do SELECT")
